...
void paintMagicFx(String id, Canvas c, Size s, FxFrame f, double t) {
  ...
  switch (id) {
    ...
    case 'rmouth':
      _rainbowMouth(c, s, f, cx, cy, fw, fh, t, hasFace);
    case 'snow':
      _snow(c, s, t);
    case 'bubbles':
      _bubbles(c, s, t);
...
void _hearts(Canvas c, Size s, double cx, double cy, double fw, double fh,
  ...
  for (var i = 0; i < 12; i++) {
    ...
    final color = _heartColors[i % _heartColors.length]
        .withValues(alpha: (1 - cycle) * 0.9);
    c.drawPath(heartPath(Offset(x, y), r), pf(color));
  }
}
...
void _rainbowMouth(Canvas c, Size s, FxFrame f, double cx, double cy,
    double fw, double fh, double t, bool hasFace) {
  if (!hasFace || f.mouth < 0.2) return;
  final mx = (f.nx == 0 ? cx : f.nx * s.width);
  final my = (f.ny == 0 ? cy + fh * 0.10 : f.ny * s.height) + fh * 0.16;
  const steps = 8;
  ...
  final centers = List.generate(steps + 1, (k) {
    ...
    final half = fw * (0.16 * f.mouth + 0.34 * p);
...
void _bubbles(Canvas c, Size s, double t) {
  ...
  for (var i = 0; i < 16; i++) {
    ...
    c.drawCircle(Offset(x, y), r,
        pf(Colors.white.withValues(alpha: 0.10 * alpha)));
    c.drawCircle(Offset(x, y), r,
        ps(Colors.white.withValues(alpha: 0.65 * alpha), r * 0.12));
    c.drawCircle(Offset(x - r * 0.35, y - r * 0.35), r * 0.18,
        pf(Colors.white.withValues(alpha: 0.8 * alpha)));
  }
}
...
void _confetti(Canvas c, Size s, double t) {
  ...
  for (var i = 0; i < 28; i++) {
    ...
    c.translate(x, y);
    c.rotate(angle);
    // Dönerken daralıp genişler: 3B takla izlenimi
    ...
    c.drawRect(
      ...
      pf(_confettiColors[i % _confettiColors.length].withValues(alpha: 0.95)),
    );
    c.restore();
  }
}
